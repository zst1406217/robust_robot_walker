...
class TerrainPerlin:
    def __init__(self, cfg, num_envs):
        ...
        for difficulty in range(cfg.num_rows):
            self.heightsamples_float = self.generate_fractal_noise_2d(self.xSize//cfg.num_rows, self.ySize, self.tot_rows//cfg.num_rows, self.tot_cols, cfg.num_rows, difficulty, **cfg.TerrainPerlin_kwargs)
            self.heightsamples_float = (self.heightsamples_float * (1 / cfg.vertical_scale)).astype(np.int16)
            self.heightsamples[difficulty*self.tot_rows//cfg.num_rows:(difficulty+1)*self.tot_rows//cfg.num_rows,:]=self.heightsamples_float
    ...
    @staticmethod
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, num_rows=4, difficulty=0,\
        frequency=10, fractalOctaves=2, fractalLacunarity = 2.0, fractalGain=0.25, zScale = 0.23):
        zScale=zScale/(num_rows-1)*difficulty
        # zScale=0
