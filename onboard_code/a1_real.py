...
class UnitreeA1Real:
    ...
    def __init__(self,
            ...
            move_by_wireless_remote= True, # if True, command will not listen to move_cmd_subscriber, but wireless remote.
            cfg= dict(),
            extra_cfg= dict(),
        ...
        self.goal_buf = torch.zeros((self.num_envs, 4,), device= self.model_device, dtype= torch.float32) # zeros for initialization
        self.goal_buf[0, 3] =2.
    ...
    def process_configs(self):
        ...
        if self.cfg["normalization"].get("clip_action_method",None) == "hard":
            self.clip_action_method = "hard"
            self.clip_actions_low = torch.tensor(self.cfg["normalization"].get("clip_actions_low",None), device= self.model_device, dtype= torch.float32)
            self.clip_actions_high = torch.tensor(self.cfg["normalization"].get("clip_actions_high",None), device= self.model_device, dtype= torch.float32)
    ...
    def clip_action_before_scale(self, actions):
        ...
        #if self.clip_action_method == "hard":
    ...
    def _get_whole_obs(self):
        ...
        return torch.cat([
            ...
            base_ang_vel * self.obs_scales["ang_vel"],
            projected_gravity,
            self.goal_buf[:, :3] * self.commands_scale,
            (dof_pos - self.default_dof_pos) * self.obs_scales["dof_pos"],
            dof_vel * self.obs_scales["dof_vel"],
            self.actions,
            self.goal_buf[:, 3:],
            torch.zeros((1, 188), device=self.model_device)
        ], dim= -1)
    ...
    def update_low_state(self, ros_msg,max_vel):
        self.low_state_buffer = ros_msg
        if self.move_by_wireless_remote:
            self.goal_buf[0, 0] = self.low_state_buffer.wirelessRemote.ly
            self.goal_buf[0, 1] = -self.low_state_buffer.wirelessRemote.lx # right-moving stick is positive
            if self.goal_buf[0, 0]<-0.5:
                self.goal_buf[0, 0]=-0.5
            elif self.goal_buf[0, 0]>0.1 and self.goal_buf[0, 0]<1:
                self.goal_buf[0, 0]=2

            if self.goal_buf[0, 1]<0.8 and self.goal_buf[0, 1]>0.2:
                self.goal_buf[0, 1]=0.4
            elif self.goal_buf[0, 1]<-0.2 and self.goal_buf[0, 1]>-0.8:
                self.goal_buf[0, 1]=-0.4
            elif self.goal_buf[0, 1]>0.8:
                self.goal_buf[0, 1]=2
            elif self.goal_buf[0, 1]<-0.8:
                self.goal_buf[0, 1]=-2
            
            if np.linalg.norm(self.goal_buf[0, :2]) < 0.2:
                self.goal_buf[0, :2] = 0.
                self.goal_buf[0, 3]=0
            else:
                self.goal_buf[0, 3]=4
